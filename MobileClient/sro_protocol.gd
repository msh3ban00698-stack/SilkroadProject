...
signal entity_despawned(unique_id: int)
signal entity_moved(unique_id: int, movement: Dictionary)
signal target_info_received(target: Dictionary)
signal action_result_received(result: Dictionary)
signal item_picked_up(result: Dictionary)
...
const OP_AGENT_SELECT := 0x7045
const OP_AGENT_ACTION := 0x7074
const OP_AGENT_ACTION_RESULT := 0xB074
const OP_AGENT_PICKUP := 0x7099
const OP_AGENT_PICKUP_RESULT := 0xB099
const OP_AGENT_SPAWN_SINGLE := 0x3015
const OP_AGENT_DESPAWN_SINGLE := 0x3016
const OP_AGENT_SPAWN_GROUP_START := 0x3017
const OP_AGENT_SPAWN_GROUP_END := 0x3018
const OP_AGENT_SPAWN_GROUP_DATA := 0x3019
const OP_AGENT_MOVEMENT := 0xB021
const OP_AGENT_SELECT_RESULT := 0xB045
const OP_AGENT_DROP_ITEM := 0x304D
...
func _handle_packet(opcode: int, payload: PackedByteArray) -> void:
    match opcode:
        ...
        OP_AGENT_WORLD_READY:
            ...
            world_ready.emit()
        OP_AGENT_SPAWN_SINGLE:
            _handle_spawn(payload, true)
        OP_AGENT_SPAWN_GROUP_DATA:
            _handle_spawn(payload, false)
        OP_AGENT_DESPAWN_SINGLE:
            _handle_despawn(payload)
        OP_AGENT_MOVEMENT:
            _handle_entity_movement(payload)
        OP_AGENT_SELECT_RESULT:
            _handle_target_result(payload)
        OP_AGENT_ACTION_RESULT:
            _handle_action_result(payload)
        OP_AGENT_PICKUP_RESULT:
            _handle_pickup_result(payload)
        OP_AGENT_DROP_ITEM:
            status_changed.emit("Drop notification received")
        _:
...
func select_entity(unique_id: int) -> void:
    var writer := PacketWriter.new()
    writer.write_i32(unique_id)
    _send_encrypted(OP_AGENT_SELECT, writer.data)

func attack_target(unique_id: int, skill_id: int = 0) -> void:
    var writer := PacketWriter.new()
    writer.write_i32(unique_id)
    writer.write_i32(skill_id)
    _send_encrypted(OP_AGENT_ACTION, writer.data)
    status_changed.emit("Attack action sent")

func pickup_item(unique_id: int) -> void:
    var writer := PacketWriter.new()
    writer.write_i32(unique_id)
    _send_encrypted(OP_AGENT_PICKUP, writer.data)
    status_changed.emit("Pickup request sent")
...
func _handle_spawn(payload: PackedByteArray, single: bool) -> void:
    var reader := PacketReader.new(payload)
    if reader.remaining() < 8:
        return
    var entity := _read_spawn_entity(reader, single, payload.size())
    if not entity.is_empty():
        entity_spawned.emit(entity)
        status_changed.emit("Spawned %s 0x%08X" % [entity.kind, entity.unique_id])

func _read_spawn_entity(reader: PacketReader, single: bool, payload_size: int) -> Dictionary:
    if reader.remaining() < 4:
        return {}
    var entity := {"model": reader.read_u32(), "kind": "monster", "unique_id": 0, "region": 0, "position": Vector3.ZERO, "angle": 0, "hp": 100, "max_hp": 100, "rarity": 0, "life_state": 1}
    if single and payload_size < 42:
        entity.kind = "item"
        if reader.remaining() >= 4:
            entity.unique_id = reader.read_i32()
        if reader.remaining() >= 2:
            entity.region = reader.read_u16()
        if reader.remaining() >= 12:
            entity.position = Vector3(reader.read_f32(), reader.read_f32(), reader.read_f32())
        if reader.remaining() >= 2:
            entity.angle = reader.read_u16()
        entity.hp = 1
        entity.max_hp = 1
        return entity
    if reader.remaining() < 4 + 2 + 12 + 2:
        return {}
    entity.unique_id = reader.read_i32()
    entity.region = reader.read_u16()
    entity.position = Vector3(reader.read_f32(), reader.read_f32(), reader.read_f32())
    ...
    if reader.remaining() > 0:
        var has_destination := reader.read_u8()
        entity.has_destination = has_destination
        if has_destination != 0 and reader.remaining() >= 8:
            entity.destination_region = reader.read_u16()
            entity.destination = Vector3(reader.read_u16(), reader.read_u16(), reader.read_u16())
        elif reader.remaining() >= 3:
            entity.has_angle_movement = reader.read_u8()
            entity.angle = reader.read_u16()
    if reader.remaining() >= 3:
        entity.life_state = reader.read_u8()
        entity.movement_type = reader.read_u8()
        entity.status = reader.read_u8()
    if reader.remaining() >= 12:
        entity.walk_speed = reader.read_f32()
        entity.run_speed = reader.read_f32()
        entity.hwan_speed = reader.read_f32()
    if reader.remaining() > 0:
        var buff_count := reader.read_u8()
        reader.skip(min(buff_count * 8, reader.remaining()))
    if reader.remaining() >= 2:
        reader.read_u8()
        entity.rarity = reader.read_u8()
    if single and reader.remaining() > 0:
        reader.read_u8()
    return entity

func _handle_despawn(payload: PackedByteArray) -> void:
    if payload.size() >= 4:
        var reader := PacketReader.new(payload)
        entity_despawned.emit(reader.read_i32())

func _handle_entity_movement(payload: PackedByteArray) -> void:
    var reader := PacketReader.new(payload)
    if reader.remaining() < 8:
        return
    var unique_id := reader.read_i32()
    var mode := reader.read_i32()
    var movement := {"mode": mode, "position": Vector3.ZERO, "destination": Vector3.ZERO, "angle": 0}
    if mode == 1 and reader.remaining() >= 2 + 6 + 2 + 12 + 2:
        movement.region = reader.read_u16()
        movement.destination = Vector3(reader.read_u16(), reader.read_u16(), reader.read_u16())
        movement.source_region = reader.read_u16()
        movement.position = Vector3(reader.read_f32(), reader.read_f32(), reader.read_f32())
        reader.read_u8()
        reader.read_u8()
    elif reader.remaining() >= 2 + 12 + 2:
        movement.region = reader.read_u16()
        movement.position = Vector3(reader.read_f32(), reader.read_f32(), reader.read_f32())
        movement.angle = reader.read_u16()
    entity_moved.emit(unique_id, movement)

func _handle_target_result(payload: PackedByteArray) -> void:
    var reader := PacketReader.new(payload)
    if reader.remaining() < 5:
        return
    var accepted := reader.read_u8() == 1
    var target_id := reader.read_i32()
    var target := {"accepted": accepted, "unique_id": target_id, "hp": 0, "max_hp": 100}
    if accepted and reader.remaining() >= 5:
        reader.read_u8()
        target.hp = reader.read_i32()
    target_info_received.emit(target)

func _handle_action_result(payload: PackedByteArray) -> void:
    var reader := PacketReader.new(payload)
    if reader.remaining() < 17:
        return
    var result := {"target_id": reader.read_i32(), "skill_id": reader.read_i32(), "success": reader.read_u8() == 1, "damage": reader.read_i32(), "hp": reader.read_i32(), "dead": reader.read_u8() == 1}
    action_result_received.emit(result)

func _handle_pickup_result(payload: PackedByteArray) -> void:
    var reader := PacketReader.new(payload)
    if reader.remaining() < 9:
        return
    var result := {"unique_id": reader.read_i32(), "success": reader.read_u8() == 1, "item_id": reader.read_i32()}
    item_picked_up.emit(result)
...
class PacketReader:
    ...
    func skip(count: int) -> void:
        offset = min(data.size(), offset + count)
