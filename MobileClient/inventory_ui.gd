...
func _build() -> void:
    ...
    panel.offset_left = -_px(180)
    panel.offset_right = _px(180)
    panel.offset_top = -_px(235)
    panel.offset_bottom = _px(235)
    panel.mouse_filter = Control.MOUSE_FILTER_STOP
    add_child(panel)
    var content := VBoxContainer.new()
    content.add_theme_constant_override("separation", _px(12))
    ...
    title.add_theme_font_size_override("font_size", _fs(24))
    title.add_theme_color_override("font_color", Color("#f2c66d"))
    header.add_child(title)
    var close := Button.new()
    close.text = "X"
    close.custom_minimum_size = Vector2(_px(56), _px(44))
    close.add_theme_font_size_override("font_size", _fs(18))
    ...
    list.add_theme_constant_override("separation", _px(7))
...
func _refresh() -> void:
    ...
    if items.is_empty():
        ...
        empty.add_theme_font_size_override("font_size", _fs(16))
        list.add_child(empty)
        return
    for index in range(items.size()):
        var row := Label.new()
        row.text = "%02d   %s   [0x%08X]" % [index + 1, items[index].name, int(items[index].id)]
        row.add_theme_font_size_override("font_size", _fs(16))
